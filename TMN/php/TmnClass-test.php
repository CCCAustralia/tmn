...
fb("Database Test");
fb("disconnect");
$tmnObj->disconnectFromDatabase();
fb("connect");
$tmnObj->connectToDatabase();
...
fb($values);
fb($types);
$session_id	= $tmnObj->preparedQuery($sql, $values, $types);
fb($session_id);
...
fb($types);
fb($resultTypes);
fb($tmnObj->preparedSelect($sql, $values, $types, $resultTypes));
...
fb($values);
fb($types);
fb($tmnObj->preparedQuery($sql, $values, $types));
