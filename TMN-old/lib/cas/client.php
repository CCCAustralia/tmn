...
//	    $res = FALSE;
  ...
  function isAuthenticated() {
		...
		if ( $this->wasPreviouslyAuthenticated() ) {
			...
			$_SESSION['phpCAS']['serviceResponse'] = $this->getServiceResponse();
  ...
  function wasPreviouslyAuthenticated() {
		...
		if ( $this->isProxy() ) {
			...
			if ( $this->isSessionAuthenticated() && !empty($_SESSION['phpCAS']['pgt']) ) {
				...
				$this->setServiceResponse($_SESSION['phpCAS']['serviceResponse']);
  ...
  function redirectToCas($gateway=false)
    {
      phpCAS::traceBegin();
//change! OpenFeel Pte Ltd 20080218 - added headers_sent check. There seems to be issue of "headers already sent" warning when using callback
      if (!headers_sent()) {
      		/* File permissions for Drupal module(?) Commented out for non-Drupal installation
		  // checks whether file permissions are set correctly before logging in (16877 is equivalent to 0755; 33188 to 0644)		  
		  if (fileperms( 'sites/all/modules' ) != 16877) @chmod( 'sites/all/modules', 0755 );
		  if (fileperms( 'sites/all/modules/cas' ) != 16877) @chmod( 'sites/all/modules/cas', 0755 );
		  if (fileperms( 'sites/all/modules/cas/callback.php' ) != 33188) @chmod( 'sites/all/modules/cas/callback.php', 0644 );	
		*/	  		  
		  ...
		  phpCAS::trace('Headers already send! Can\'t redirect for cas login');
	  }
      phpCAS::traceExit();
    }
...
//change! OpenFeel Pte Ltd 20060819 - add a variable for the service response from the CAS server
  /**
   * String to store service response from the CAS server
   *
   * @private
   * @see CASClient::setServiceResponse, CASClient::getSericeResponse
   */
  var $_service_response = '';

  /**
   * This method returns the service response from the CAS server
  ...
  /**
   * This method stores the service response from the CAS server
   * @param $serviceResponse The service respose.
   * @private
   */
  function setServiceResponse($serviceResponse)
    { $this->_service_response = $serviceResponse; }
//end change!
  ...
  function validateST($validate_url,&$text_response,&$tree_response) {
		...
		$this->setServiceResponse($text_response);
...
function proxyURL($url, &$cookies, &$body, &$err_msg, $redirectCount)
{
	phpCAS::traceBegin();
	$MAX_REDIRECTS = 10;
	$body = '';
	...
	if($redirectCount > $MAX_REDIRECTS) {
		$err_msg = 'getURL Exceeded MAX_REDIRECTS ('.$MAX_REDIRECTS.')';
		...
		if ($this->_cas_server_cert != '' ) {
			...
			curl_setopt($request, CURLOPT_SSLCERT, $this->_cas_server_cert);
		} else if ($this->_cas_server_ca_cert != '') {
			curl_setopt($request, CURLOPT_SSL_VERIFYPEER, 1);
			curl_setopt($request, CURLOPT_CAINFO, $this->_cas_server_ca_cert);
		} else {
		...
		// add cookies headers
		if ( is_array($cookies) ) {
			curl_setopt($request,CURLOPT_COOKIE,implode(';',$cookies));
		}
		$response = curl_exec ($request);
		if ( $response === FALSE ) {
			phpCAS::trace('curl_exec() failed');
			$err_msg = 'CURL error #'.curl_errno($request).': '.curl_error($request);
			// close the CURL session
			curl_close ($request);
			$res = FALSE;
		} else {
			curl_close ($request);
			$pos = stripos($response, chr(13).chr(10).chr(13).chr(10));
			if($pos == FALSE) {
				...
				foreach ( $headers as $header ) {
					...
					} else if( preg_match('/^Location:/',$header) ) {
						$location = preg_replace('/^Location: */','',$header);
					}
				}
				...
				if(300 <= $responseCode && $responseCode <= 303) {
					...
					if( !$this->proxyURL($location, $cookies, $body, $err_msg, $redirectCount++) ) {
						// give an error message
						$err_msg = $location.": FAILED";
						$res = FALSE;
					}
				}
			}
  ...
  function serviceWeb($url,&$err_code,&$output) {
	...
	phpCAS::trace('reading URL`'.$service_url.'\'');
		if( !$this->proxyURL($service_url, $cookies, $output, $err_msg, 0) ) {
  ...
  function getURL()
    {
        // OpenFeel Pte Ltd 20061123 - comment out the traceBegin function call
        // Previously, 500 Server Error is shown. This is caused by debug_backtrace() called within
        // the function.
      phpCAS::traceBegin();
      // the URL is built when needed only
      if ( empty($this->_url) ) {
	  ...
	  $this->setURL($final_uri);
       ...
       // OpenFeel Pte Ltd 20061123 - comment out the traceEnd function call
